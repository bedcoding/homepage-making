...
<%
	if (request.getAttribute("prev") != null) 
	{
%>		<a href="./BoardDetailAction.bo3?num=<%=p%>"><%=s%></a> <%
    }
...
%>
 ...
 <a href="./BoardDetailAction.bo3?num=<%=n%>"><%=sn%></a> 
<%
    }
			  ...
			  <td>
			   <table width="260" height="100"  border="0" cellpadding="0" cellspacing="0" align="center" style="background-color:#fff">
				...
				%> 		
						<div class="board_btn">
							<p class="board_fLeft">
								<input type="button" class="board_bt_style01" title="수정하기" value="수정하기" name="" id="write_img" onClick="location='./BoardModify.bo3?num=<%=board.getBOARD_NUM() %>' " />
								<input type="button" class="board_bt_style02" title="삭제하기" value="삭제하기" name="" id="del_img" onClick="location='./BoardDelete.bo3?num=<%=board.getBOARD_NUM() %>' "/>
							</p>
							<p class="board_fRight"><input type="button" class="board_bt_style01" title="목록으로" value="목록으로" name="" onClick="location='./BoardList.bo3'"/></p>
						</div>
				<%  } 
